...
// schema: TEST_SCHEMA
var customSchema = Schema.create(yaml.DEFAULT_SAFE_SCHEMA, [
  testType1,
  testType2,
  testType3,
  testType4,
...
suite('YAML 1.2 Test Suite', function () {
  ...
  fs.readdirSync(samplesDir).forEach(function (specFile) {
    ...
    var m = re.exec(spec);
    if (!m) {
      throw new Error('yaml-test-suite TML regex does not parse the spec file ' + specFile +
          ', please check and augment the regex');
    }
    ...
    if (typeof inYaml !== 'string') {
      throw new Error('yaml-test-suite TML spec file ' + specFile +
          ' did not produce an INPUT YAML section. Please check and fix the test (or test rig).');
    }
